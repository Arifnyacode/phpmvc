        ...
        <div class="col-6">
            ...
            <?php foreach($data['siswa'] as $sw):?>
                <li class="list-group-item">
                    <?=$sw['nama'];?>
                    <a href="<?=BASEURL;?>/siswa/hapus/<?=$sw['id'];?>" class="btn btn-danger btn-sm float-right ml-1" onclick="return confirm('Anda Yakin ingin menghapus?')">hapus</a>
                    <a href="<?=BASEURL;?>/siswa/ubah/<?=$sw['id'];?>" class="btn btn-success btn-sm float-right ml-1 tampilModalUbah" data-toggle="modal" data-target="#formModal" data-id="<?=$sw['id']?>">ubah</a>
                    <a href="<?=BASEURL;?>/siswa/detail/<?=$sw['id'];?>" class="btn btn-primary btn-sm float-right ml-1">detail</a>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="form-group">
			...
			<input type="text" class="form-control" id="nama" name="nama" placeholder="isikan nama" autocomplete="off">
	  	</div>
	  	<div class="form-group">
	  	...
	  	<div class="form-group">
			...
			<input type="email" class="form-control" id="email" name="email" placeholder="isikan email" autocomplete="off">
	  	</div>
      <div class="form-group">
      <label for="jurusan" class="form-label">Jurusan</label>
		  ...
		  <option value="Rekayasa Perangkat Lunak">Pengembangan Perangkat Lunak dan Gim (PPLG)</option>
		</select>
      </div>
      </div>
